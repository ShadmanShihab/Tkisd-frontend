  ...
  return (
    <div>
      <h2 id="orders-heading" data-cy="OrdersHeading">
        <Translate contentKey="tkisdApp.orders.home.title">Orders</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="tkisdApp.orders.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/orders/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="tkisdApp.orders.home.createLabel">Create new Orders</Translate>
          </Link>
        </div>
      </h2>
        ...
        {ordersList && ordersList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="tkisdApp.orders.id">Id</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('amount')}>
                  <Translate contentKey="tkisdApp.orders.amount">Amount</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('amount')} />
                </th>
                <th className="hand" onClick={sort('trxType')}>
                  <Translate contentKey="tkisdApp.orders.trxType">Trx Type</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('trxType')} />
                </th>
                ...
                <th>
                  <Translate contentKey="tkisdApp.orders.courseId">Course Id</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {ordersList.map((orders, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{orders.amount}</td>
                  <td>
                    <Translate contentKey={`tkisdApp.TrxType.${orders.trxType}`} />
                  </td>
                  <td>{orders.userId}</td>
                  <td>{orders.courseId ? <Link to={`/courses/${orders.courseId.id}`}>{orders.courseId.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="tkisdApp.orders.home.notFound">No Orders found</Translate>
            </div>
          )
        )}
      </div>
